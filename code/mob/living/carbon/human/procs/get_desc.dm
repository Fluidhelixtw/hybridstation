...
*/
	...
	if ((isdead(src)) || changeling_fakedeath || src.bioHolder?.HasEffect("dead_scan") == 2 || (src.reagents.has_reagent("capulettium") && src.getStatusDuration("paralysis")) || (src.reagents.has_reagent("necrotonium") && src.hasStatus("resting")))
		if (!src.decomp_stage)
